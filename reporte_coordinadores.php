...
function Row($data)
   ...
   $this->CheckPageBreak($h);
	...
	function Header()
	{
	    ...
	    $this->Text(75,30,'Listado de Coordinadores de Centros de Votacion',0,'C', 0);
    $this->Ln(13);
	}
...
// Para realizar esto utilizaremos la funcion Row()
	...
	$pdf->SetWidths(array(50,25,25,50,50)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetro15s
	$pdf->SetDrawColor(210,0,0);
	$pdf->Row(array('Nombre y Apellido', 'Cedula', 'Telefono', 'Centro', 'Direccion')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita
	$pdf->SetFont('Times','',9);
	$pdf->SetDrawColor(210,0,0);
	...
	$strConsulta2 = "SELECT * FROM coordinadores";
...
for ($i=0; $i<$numfilas; $i++)   {       
	...
	$fila = mysql_fetch_array($eletores);                  
  	 ...
  	 mysql_select_db($database_conexion, $conexion);
		$query_centros = "SELECT * FROM centro_votacion where id_centro='$fila[centr]'";
	...
	$pdf->Row(array($fila['nombres'], $fila['cedula'], $fila['telefono'], $row_centros['nombre'], $fila['direccion']));
		}

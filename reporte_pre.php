...
function Row($data)
   ...
   $this->CheckPageBreak($h);
	...
	function Header()
	{
	    ...
	    $this->Text(75,30,'Diagnostico Pre-Electoral',0,'C', 0);
    $this->Ln(13);
	}
...
// Para realizar esto utilizaremos la funcion Row()
	...
	$pdf->SetWidths(array(40,15,15,15,50,12,12,20,10)); // Definimos el tamaño de las columnas, tomando en cuenta que las declaramos en milimetros, ya que nuestra hoja esta en milimetro15s
	$pdf->SetDrawColor(210,0,0);
	$pdf->Row(array('Nombre y Apellido', 'Cedula', 'Telefono', 'Situacion', 'Centro de Votacion', 'codigo','mesa','Diagnostico', 'CPE')); // creamos nuestra fila con las columnas fecha(fecha de la visita al medico), medico(nombre del medico que nos atendio), consultorio y el diagnostico en esa visita
	$pdf->SetFont('Times','',9);
	$pdf->SetDrawColor(210,0,0);
...
for ($i=0; $i<$numfilas; $i++)   {       
	...
	$strConsulta3 = "SELECT * FROM diagnostico_pre where elector='$fila[cedula]'";
	...
	$pdf->Row(array($cadena, $fila['cedula'], $fila['telefono'], $fila2['situacion'], $fila['centro'], $fila3['codigo'], $fila['mesa'], $fila2['diagnostico'], $fila2['CPE']));
		}
